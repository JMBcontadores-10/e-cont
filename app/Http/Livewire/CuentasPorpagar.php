    ...
    public function render()
    {
        ...
        if (!empty(auth()->user()->tipo)) {
            ...
            for ($i = 0; $i < $largo; $i++) {
                ...
                foreach ($e as $em)
                    $emp[] = array(strtoupper($em['RFC']), $em['nombre']);
            }
        ...
        $col = MetadataR::search($this->search)
        ...
        $provselect = MetadataR::select('emisorNombre', 'emisorRfc')
            ->where('receptorRfc',  $this->rfcEmpresa)
            ->where('estado', '<>', 'Cancelado')
            ->whereNull('cheques_id')
            ->groupBy('emisorRfc')
        ...
        if (is_array($this->RFC)) {
            //Si es arreglo
            $CFDI = MetadataR::searchxml($this->searchcfdi)
                ->where('estado', '<>', 'Cancelado')
                ->where('receptorRfc', $this->rfcEmpresa)
        ...
        $Cheques = Cheques::where('rfc', $this->rfcEmpresa)
            ->where('verificado', '=', 0)
            ->where('conta', '=', 0)
            ->orderBy('fecha', 'desc')
            ->get();
