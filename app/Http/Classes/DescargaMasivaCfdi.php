...
class DescargaMasivaCfdi
{
    ...
    {
        // 1
        // $respuesta = RespuestaCurl::request('https://cfdiau.sat.gob.mx/nidp/app/login?id=SATUPCFDiCon&sid=0&option=credential&sid=0');
    ...
    {
        if (get_class($filtros) == 'App\Http\Classes\BusquedaEmitidos') {
            $url = 'https://portalcfdi.facturaelectronica.sat.gob.mx/ConsultaEmisor.aspx';
            $modulo = 'emitidos';
        } elseif (get_class($filtros) == 'App\Http\Classes\BusquedaRecibidos') {
    ...
    private function getFormData($html)
    {
        ...
        foreach (array('input', 'select') as $element) {
            // foreach ($form->getElementsByTagName($element) as $val) {
            foreach ($document->getElementsByTagName($element) as $val) {
                $name = $val->getAttribute('name');
                if (!empty($name)) {
                    $post[$name] = utf8_decode($val->getAttribute('value'));
                }
...
class BusquedaRecibidos
{
    ...
    public function obtenerFormulario()
    {
        ...
        return array(
            ...
            'ctl00$MainContent$FiltroCentral' => ($this->tipoBusqueda == 'fecha') ? 'RdoFechas' : 'RdoFolioFiscal'
        );
    }
...
class MultiCurl
{
    ...
    public function addRequest($url, $user_data = null)
